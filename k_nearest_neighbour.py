...
# We will use the digits dataset as an example. It consists of the
# 1797 images of hand_written digits. Each digit is represented by a
# 64-dimensional vector of pixel values
...
class kNN():
    ...
    def predict(self, X, k=1):
        """
        Predict the classification for an input example or matrix of
        input examples X
        :param X:
        :param k:
        :return:
        ...
        # Step 2: find the k nearset neighbors and their classification
        if X.ndim == 1:
            if k == 1:
                nn = np.argmin(dists)
...
print(f"Testing accuracy with k = 5: {format(test_acc5)}")