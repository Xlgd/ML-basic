...
y_true = y[:, np.newaxis]
# Split with data into a training and test set
X_train, X_test, y_train, y_test = train_test_split(X, y_true)

print(f'Shape X_train: {X_train.shape}')
print(f'Shape y_trian: {y_train.shape}')
...
class NeuralNet():
    ...
    def cost(self, y_true, y_predict, n_samples):
        """
        Computes and reurns the cost over all examples
        :param y_true:
        :param y_predict:
        :param n_samples:
        :return:
    ...
    def predict(self, X):
        """
        Computes and returns network predictions for given data set
        :param X:
        :return:
        """
        outputs = self.forward_pass(X)
